...
const createBikeFunc = async (data) => {
    if (!data?.customerId)
    ...
    if (!data?.model)
        throw new AppError(StatusCodes.BAD_REQUEST, "Model is required");
    if (!data?.year)
        throw new AppError(StatusCodes.BAD_REQUEST, "Year is required");
...
const getBikeByIdFunc = async (id) => {
    if (!id)
        throw new AppError(StatusCodes.NOT_FOUND, "Id not found");
