...
const createCustomerFunc = async (data) => {
    if (!data?.name)
        throw new AppError(StatusCodes.BAD_REQUEST, "Name is required");
    if (!data?.email)
        throw new AppError(StatusCodes.BAD_REQUEST, "Email is required");
    if (!data?.phone)
        throw new AppError(StatusCodes.BAD_REQUEST, "Phone is required");
    ...
    if (isExistEmail)
        throw new AppError(StatusCodes.CONFLICT, "Email is already exist");
...
const getCustomerByIdFunc = async (id) => {
    if (!id)
...
const updateCustomerByIdFunc = async (id, data) => {
    if (!id)
        throw new AppError(StatusCodes.NOT_FOUND, "Id not found");
    ...
    if (!isExitUser)
        throw new AppError(StatusCodes.NOT_FOUND, "user not found");
    if (!data)
        throw new AppError(StatusCodes.NOT_FOUND, "Update data not found");
    const updateDoc = {};
    if (data?.name)
        updateDoc.name = data?.name;
    if (data?.phone)
        updateDoc.phone = data?.phone;
...
const deleteCustomerByIdFunc = async (id) => {
    if (!id)
        throw new AppError(StatusCodes.NOT_FOUND, "Id not found");
    const isExitUser = await prisma.customer.findUnique({
        where: {
