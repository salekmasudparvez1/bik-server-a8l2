...
const createBikeServiceFunc = async (data) => {
    if (!data?.bikeId)
        throw new AppError(StatusCodes.NOT_FOUND, "Bike Id not found");
    if (!data?.serviceDate)
        throw new AppError(StatusCodes.BAD_REQUEST, "Service date is required");
    if (!data?.description)
...
const getBikeServiceByIdFunc = async (id) => {
    if (!id)
...
const addComplitionDateFunc = async (id, completionDate) => {
    if (!id)
        throw new AppError(StatusCodes.NOT_FOUND, "Id not found");
